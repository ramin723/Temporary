  ...
  <div :key="String($route.fullPath)" class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="settlement" class="space-y-6">
        ...
        <!-- Vendor Info Card -->
        <div class="bg-white shadow rounded-lg p-6">
          <h2 class="text-lg font-medium text-gray-900 mb-4">اطلاعات فروشگاه</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <dt class="text-sm font-medium text-gray-500">نام فروشگاه</dt>
              <dd class="mt-1 text-lg text-gray-900">{{ settlement.vendor.name }}</dd>
            </div>
            <div>
              <dt class="text-sm font-medium text-gray-500">شهر</dt>
              <dd class="mt-1 text-lg text-gray-900">{{ settlement.vendor.city || 'نامشخص' }}</dd>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white shadow rounded-lg p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <dt class="text-sm font-medium text-gray-500">سهم من</dt>
        ...
        <div class="bg-white shadow rounded-lg">
          <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">تراکنش‌های تسویه شده</h3>
...
definePageMeta({
  ...
  key: (r) => `mechanic-settlement-${r.params.id}`,
})
...
interface MechanicSettlementDetail {
  id: number
  vendor: {
    id: number
    name: string
...
const { data: settlement, pending: loading, error, refresh } = await useFetch<MechanicSettlementDetail>(
  () => `/api/settlements/${route.params.id}`,
  {
    key: () => `fetch-mechanic-settlement-${route.params.id}`,
  }
)
